...
### Formatting the Exported JSON for Easier Inspection

The Neo4j export generated by `apoc.export.json.all` is written as a single-line JSON file. While this format is valid and suitable for processing, it can be difficult to read and navigate manually.

If you need to inspect or browse the graph structure, you can use the `format_json.py` script to generate a formatted version of the same JSON file with proper indentation and structure.
...
This will produce a human-readable JSON file that preserves the original data while making nodes, relationships, and properties easier to explore and debug.
